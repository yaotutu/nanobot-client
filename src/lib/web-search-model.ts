...
import type { ToolProgressEvent } from '@/types/nanobot';

import i18n from '@/i18n';
import { redactActivityText, safeActivityDetail } from '@/lib/activity-text';
import { displayWebHost, formatCompactWebUrl, parseSafeActivityHttpUrl } from '@/lib/web-url';
...
export function presentWebSearchAction(
  ...
  const verb = status === "error"
    ? i18n.t('message.webSearchFailed', { defaultValue: 'Search failed' })
    : status === "running"
      ? i18n.t('message.webSearchRunning', { defaultValue: 'Searching' })
      : i18n.t('message.webSearchDone', { defaultValue: 'Searched' });
...
function readableError(error: unknown): string | undefined {
  ...
  try {
    ...
    return i18n.t('message.webSearchError', { defaultValue: 'Web search failed' });
  }
}
