...
import { compactActivityPath, redactActivityText } from "@/lib/activity-text";

export type GenericToolStatus = "running" | "done" | "error";
export type ToolFamily =
  "content-search" | "file-search" | "list" | "read" | "memory" | "generic";
...
export function parseGenericToolTrace(line: string): GenericToolTrace | null {
  ...
  const collectedSource = fields.some((field) =>
    isCollectedSourcePath(field.value),
  );
  return {
    name: call.name,
    family,
