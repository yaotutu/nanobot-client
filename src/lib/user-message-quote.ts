...
export const MAX_QUOTED_CONTEXT_CHARS = 4_000;

function normalizeNewlines(value: string): string {
  return value.replace(/\r\n?/g, '\n');
}

export function normalizeQuotedContext(value?: string | null): string {
...
export function formatQuotedUserMessage(content: string, quotedContext?: string | null): string {
  ...
  const quote = normalizeQuotedContext(quotedContext);
...
export function parseQuotedUserMessage(content: string): ParsedUserMessageQuote {
  ...
  const quoteBlock = separatorIndex === -1
